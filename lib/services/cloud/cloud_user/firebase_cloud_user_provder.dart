...
class FirebaseCloudUserProvider extends CloudUserProvider {
  ...
  Future<CloudUser?> getUser({
    ...
    final document =
        await firebaseCloudStorage.firebaseFirestoreInstance().collection(_users).doc(userId).get();
  ...
  Future<List<CloudUser>> searchUsers(String keyword) {
    if (keyword.isNotEmpty) {
      ...
      return firebaseCloudStorage.firebaseFirestoreInstance().collection(_users).get().then(
            (querySnapshot) => querySnapshot.docs
                .map(
                  (doc) => CloudUser.fromSnapshot(doc),
                )
  ...
  Future<bool> followUser(CloudUser currentUser, CloudUser followUser) async {
    ...
    try {
      ...
      if (followUser.follower.contains(currentUser.userId)) {
        firebaseCloudStorage.firebaseFirestoreInstance().collection(_users).doc(followUser.userId).update({
          'follower': FieldValue.arrayRemove([currentUser.userId])
        });
        firebaseCloudStorage.firebaseFirestoreInstance().collection(_users).doc(currentUser.userId).update({
